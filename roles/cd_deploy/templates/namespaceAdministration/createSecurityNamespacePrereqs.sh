...
if supports_scc; then
    ...
    # Note: this script only works on OpenShift >= 3.11, otherwise you must run the following command manually
      oc adm policy add-scc-to-group ibm-connect-direct-scc system:serviceaccounts:$namespace
    else
...
if supports_psp; then
    ...
    sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-cdu-rb.yaml > $namespace-ibm-cdu-rb.yaml
    ...
    kubectl apply -f $namespace-ibm-cdu-rb.yaml -n $namespace
